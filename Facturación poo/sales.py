...
class Sale(Icalculo):
    ...
    def __init__(self,client):
        id_sale=read('file/id.json')
        Sale.next=id_sale['id_factura']
        Sale.next +=1
        self.__invoice =('F0' if id_sale['id_factura'] <10 else 'F')+str(Sale.next)
        ...
        self.total = 0
        self.sale_detail = []
        id_sale['id_factura']=Sale.next
        save(id_sale,'file/id.json')
    @property
    def invoice(self):
        # Getter para obtener el valor del límite de crédito del cliente VIP
